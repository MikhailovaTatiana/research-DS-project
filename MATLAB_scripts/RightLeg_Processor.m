% --- MATLAB Script: RightLeg_Processor.m ---

% Configuration
DATA_ROOT_FOLDER = 'Raw_Data';
TABLE_EMG = 'RightLeg_EMG';  
IMU_STRUCTURE_NAME = 'RightLeg_IMU'; 
...
MAJOR_LEG_PATHS = {'RightFoot_GaitCycle_Data'}; 
...
OUTPUT_FILENAME = 'AllParticipants_RightLeg_EMG_IMU.csv'; 
...
for i = 1:length(PARTICIPANTS)
    ...
    try
        ...
        try
            ...
            fprintf('  has RightFoot_GaitCycle_Data: %d\n', isfield(data_struct,'RightFoot_GaitCycle_Data'));
        catch
        end
        ...
        for k = 1:length(MAJOR_LEG_PATHS)
            ...
            % IMU is optional; if missing, we shall proceed with EMG-only for this segment
            ...
            try
                ...
                elseif isstruct(raw_emg)
                    % If it is a struct array, struct2table produces rows; if scalar struct, still yields a single row with array vars
